...
export function GroupLessonsTab({ groupId }: { groupId: string }) {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const [h, active] = await Promise.all([apiClassHistory(groupId), apiActiveClassSessions()]);
      setHistory(h);
      setActiveId(active.find((s) => s.groupId === groupId)?.id ?? null);
    } catch {
  ...
  const handleStart = async () => {
    ...
    setStarting(true);
    try {
      const { id } = await apiCreateClassSession(groupId);
        ...
        <div className="flex flex-col divide-y divide-gray-100">
          {history.map((h) => (
            <button
              ...
              className="flex flex-wrap items-center gap-x-4 gap-y-1 py-3 text-left hover:bg-gray-50 rounded-xl px-2"
            >
              ...
              <span className="text-xs text-emerald-600 font-medium">{h.presentCount} keldi</span>
              <span className="text-xs text-amber-600 font-medium">{h.lateCount} kech</span>
              <span className="text-xs text-gray-400 font-medium">{h.absentCount} yo'q</span>
            </button>
          ))}
        </div>
      )}
