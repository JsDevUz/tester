...
import { Film, Image as ImageIcon, Paperclip, LayoutGrid, ChevronUp, ChevronDown, X, Link2, Plus } from 'lucide-react';
...
export function ContentBlockView({ index, block, onChangeHtml, onChangeEmbedUrl, onChangeFileName, onPickFile, onRemove }: ContentBlockViewProps) {
        ...
        <div className="border-t border-gray-100 px-4 py-4">
          ...
          {(block.type === 'video' || block.type === 'image' || block.type === 'file') && (
            <div className="flex flex-col gap-3">
              ...
              {!block.embedUrl && (
                <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 py-10 transition-colors hover:border-indigo-200 hover:bg-indigo-50/30">
                  ...
                  {block.previewUrl ? (
                    block.type === 'video' ? (
                      ...
                      <span className="flex items-center gap-2 text-sm font-medium text-gray-700"><Paperclip size={16} /> {block.fileName}</span>
                    )
                  ) : (
              ...
              {block.type === 'video' && (
                <div>
                  <p className="mb-1.5 text-sm text-gray-500">Videoning nomi</p>
                  <input
                    value={block.fileName ?? ''}
                    onChange={(e) => onChangeFileName(e.target.value)}
                    placeholder="Videoning nomini kiriting"
                    className="w-full rounded-xl border-2 border-gray-100 bg-gray-50 px-4 py-2.5 text-sm outline-none focus:border-indigo-400"
                  />
                </div>
              )}
