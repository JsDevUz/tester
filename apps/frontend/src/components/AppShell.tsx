    ...
    >
        ...
        <nav className="flex flex-row lg:flex-col gap-1 flex-1 lg:w-full lg:px-2 justify-around lg:justify-start">
          {SECTIONS.map((section) => {
            ...
            const isOpen = openKey
              ? openKey === section.key
              : desktopShownSection?.key === section.key;
            return (
              <button
                key={section.key}
      ...
      {desktopShownSection?.subItems && (
        <div className="hidden lg:flex order-2 shrink-0 flex-col gap-3 w-64 min-h-0">
          ...
          <div className="bg-white rounded-2xl px-3.5 py-3 flex items-center gap-3 shrink-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-400 truncate">
                {desktopShownSection.subItems.length} ta bo'lim
              </p>
            </div>
          </div>

          {/* Sub-item ro'yxati */}
          <div className="min-h-0 bg-whiterounded-2xl p-2 flex flex-col overflow-y-auto bg-white rounded-2xl">
            {desktopShownSection.subItems.map((item, i) => {
              const ItemIcon = item.icon;
              const isActive =
                desktopShownSection.key === activeSection?.key &&
      ...
      {mobileShownSection?.subItems && (
        <div className="lg:hidden">
          <div
            ...
            onClick={() => setOpenKey("")}
          />
          <div className="order-1 fixed bottom-3 left-3 right-3 z-50 rounded-2xl bg-whiteflex flex-col py-5 px-3">
      ...
      {profileOpen && !desktopShownSection?.subItems && (
        <>
          ...
          />
          <div className="fixed z-50 bottom-3 left-3 right-3 lg:bottom-16 lg:left-3 lg:right-auto lg:w-64 bg-white rounded-2xlp-4">
