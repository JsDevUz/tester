...
export function LiveJoinPage() {
  ...
  function handleJoin(e: React.FormEvent) {
    ...
    if (!token) {
      navigate(`/login?redirect=${encodeURIComponent(`/live/join?pin=${pin}`)}`);
      return;
    }
  ...
  return (
    <div className="flex flex-col bg-white notranslate" translate="no"
      ...
      <form onSubmit={handleJoin} className="flex-1 flex flex-col items-center justify-center px-6">
        ...
        <input
          ...
          placeholder="000000"
          className="w-full max-w-xs text-center text-4xl font-black tracking-[0.3em] bg-gray-50 border-2 border-gray-100 rounded-2xl px-4 py-4 outline-none focus:border-indigo-400 focus:bg-white transition-colors mb-6"
        />
        <button type="submit" disabled={pin.length !== 6}
          ...
          <span>Kirish</span><ChevronRight size={18} />
        </button>
      </form>
    </div>
  );
