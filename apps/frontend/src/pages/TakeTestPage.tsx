...
export function TakeTestPage() {
  ...
  useEffect(() => {
    ...
    apiGetPublicTest(slug).then((t) => {
      ...
      const qsWithOpts = qs.map((q) => ({
        ...
        options:
          t.shuffleOptions && q.type !== "matching"
            ? seededShuffle(q.options, submissionId + q.id)
            : q.options,
      }));
      ...
      const savedDraft = submissionId
        ? localStorage.getItem(draftKey(submissionId))
        : null;
      if (savedDraft) {
        try {
  ...
  useEffect(() => {
    if (!submissionId || orderedQuestions.length === 0 || submittingRef.current)
      return;
