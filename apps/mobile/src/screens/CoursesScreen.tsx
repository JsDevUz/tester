...
export function CoursesScreen({navigation, route}: Props) {
  ...
  const [selectedCourseForDownloads, setSelectedCourseForDownloads] = useState<ApiMyCourse | null>(null);
    ...
    <Screen>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({item}) => {
            // Active downloads in this course
            const courseDownloads = Object.values(activeDownloads).filter(
              d => d.courseId === item.courseId,
            );
            const avgProgress =
              courseDownloads.length > 0
                ? courseDownloads.reduce((sum, d) => sum + d.progress, 0) / courseDownloads.length
                : 0;
                ...
                <Pressable
                  ...
                  <View className="mt-6 flex-row items-center justify-between">
                    ...
                    {courseDownloads.length > 0 && (
                      <DownloadCardBadge
                        size="sm"
                        count={courseDownloads.length}
                        averageProgress={avgProgress}
                        onPress={() => {
                          setSelectedCourseForDownloads(item);
      ...
      {selectedCourseForDownloads && (
        <DownloadsBottomSheet
          visible={downloadsSheetOpen}
          filterCourseId={selectedCourseForDownloads.courseId}
          title={selectedCourseForDownloads.courseTitle}
          subtitle="Yuklanayotgan videolar"
          onClose={() => setDownloadsSheetOpen(false)}
          onOpenLesson={(d) => {
            navigation.navigate('Course', {
              courseId: d.courseId || selectedCourseForDownloads.courseId,
              title: d.courseTitle || selectedCourseForDownloads.courseTitle,
              schoolId,
