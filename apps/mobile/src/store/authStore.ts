...
type AuthState={token:string|null;user:User|null;hydrated:boolean;hydrate:()=>Promise<void>;login:(phone:string,password:string)=>Promise<void>;loginCode:(code:string)=>Promise<void>;verifyPasswordResetCode:(code:string)=>Promise<{resetToken:string}>;completePasswordReset:(resetToken:string,newPassword:string,confirmPassword:string)=>Promise<void>;setUser:(user:User)=>Promise<void>;logout:()=>Promise<void>};
export const useAuthStore=create<AuthState>((set,get)=>({
  ...
  hydrate:async()=>{
    try {
      ...
      if(session?.token)api.get('/auth/me').then(r=>{const next={token:session.token,user:r.data as User};set(next);void storage.set('session',next);}).catch(()=>undefined);
    } catch {
  ...
  setUser:async user=>{const token=get().token;set({user});await storage.set('session',{token,user});},
  logout:async()=>{closePracticeMessengerSocket();await storage.remove('session');set({token:null,user:null});},
}));
